 ...
 * <p>
 * Every object contains a field "@class" with the class name.
 *
 * @author Roman Pejs
 * @version 1.0
 * @since 1.0
        ...
        private static Class<?> determineFieldClass (String className) throws Exception {
            return switch (className) {
                ...
                default -> className.startsWith("[L") && className.endsWith(";")
                        ? Array.newInstance(CacheUtil.getInstance().getClassByName(className.substring(2, className.length() - 1)), 0).getClass()
                        : CacheUtil.getInstance().getClassByName(className);
            };
        }
